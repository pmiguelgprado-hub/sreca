...
sreca/
  ...
  ingest/
    open_meteo.py              # cliente previsión irradiancia horaria (API gratis, mockeable)
  forecast/
...
## 4. Flujo de datos (1 run = un día-tipo, unidad del perfil anual)

`Open-Meteo` → `forecast/pv` (gen_h) + `forecast/demand` (dem_i,h por perfil)
→ `optimize/coefficients` (β_i,h, Σ_i=1, equidad renta-baja en escasez)
→ `optimize/load_shift` (recomendación desplazar cargas flexibles granja a ventana solar)
→ `optimize/savings` (€/hogar) → `store/db` (SQLite) → `dashboard/app` (render).
...
Sanity de calibración (Teverga 15 kWp): Σ E ≈ 16.700 kWh/año, YF ≈ 1.114 kWh/kWp,
PR ≈ 76,5 %. Test de integración compara el total anual simulado contra el de PVGIS (±10 %).
